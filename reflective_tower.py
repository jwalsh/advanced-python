    ...
    def __call__(self, *args):
        ...
        if self.evaluator:
            ...
            return self.evaluator.eval_sequence(
                self.body,
                Environment([p.name for p in self.params], args, self.env)
            )
        else:
...
class Evaluator:
    ...
    def make_global_env(self):
        ...
        env.define(Symbol("/"), lambda x, *args: x / reduce(lambda y, z: y * z, args, 1) if args else 1 / x)
        ...
        # List operations
        env.define(Symbol("cons"), lambda x, y: [x] + (y if isinstance(y, list) else [y]))
        env.define(Symbol("car"), lambda x: x[0])
        env.define(Symbol("cdr"), lambda x: x[1:])
        env.define(Symbol("list"), lambda *args: list(args))
        env.define(Symbol("null?"), lambda x: len(x) == 0 if isinstance(x, list) else False)
        ...
        env.define(Symbol("eval"), lambda x, env=None: self.eval(x, env if env else self.global_env))
        env.define(Symbol("apply"), lambda proc, args: proc(*args))
        
        # Tower operations
        env.define(Symbol("make-evaluator"), lambda: Evaluator(Environment(outer=self.global_env), self))
